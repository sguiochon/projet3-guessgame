...
public class Candidat {
    ...
    public void foundSymbol(Symbol symbol) {
        ...
        System.out.println("<<< Certitude pour " + symbol.getSymbol() + " ce qui réduit candidat à: " + toString());
    }
    ...
    public void invalidSymbol(Symbol symbol, boolean recursive) {
        ...
        System.out.println(">>> Elimination de  " + symbol.getSymbol());
        if (recursive) {
            if (potentialIncorrectPairs != null) {
                for (String pair : potentialIncorrectPairs) {
                    System.out.println("Liste de pairs de symbols peut etre incorrects " + pair);
...
/*
    public Sequence generateRandomSequence(boolean onlyUniqueSymbol) {
        LOGGER.debug("Generate random sequence based on " + this.toString());
...
public Sequence generateRandomSequence(boolean onlyUniqueSymbol) {
    ...
    for (NumberOfSymbols n : list) {
        System.out.println("je traite la colonne " + n.columnNb + " qui contient " + n.nbOfSymbols + " symbols possibles");
    ...
    public void storePotentialIncorrectPairs(Symbol symbol1, Symbol symbol2) {
        ...
        System.out.println(">>>*** Peut etre tous les deux incorrects :" + symbol1.getSymbol() + " et " + symbol2.getSymbol());
    }
    ...
    public int setCorrectSymbolsInSequence(Sequence sequence) {
        System.out.println("Si un symbol est identifié, on le place. Sequence initiale: " + sequence.toString());
        System.out.println("Choix possibles: " + this.toString());
        int nbAdjustement = 0;
        int positionNb = 0;
        for (List<String> symbols : candidatSequence) {
            if (symbols.size() == 1) {
        ...
        for (String symbol : sequence.getSymbols()){
            ...
            if (candidatSequence.get(position).size()!=1){
                ...
                if (sequence.toString().indexOf(symbol)!=sequence.toString().lastIndexOf(symbol)){
                    System.out.println("Le symbol "+ symbol + " est 2 fois dans la sequence. On va le remplacer");
                    hasDuplicated = true;
                }
            }
        }
        ...
        do{
            ...
            for (Round round : session.rounds){
                if (round.getAttempt().toString().equals(sequence.toString())){
                    isNewSequence = false;
                    System.out.println("la sequence " + sequence.toString() + " est déjà présente dans la session.");
                    break;
                }
            }
        }
